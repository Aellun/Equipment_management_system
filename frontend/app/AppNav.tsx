...
const NAV_GROUPS: NavGroup[] = [
  ...
  {
    title: "Dyzah Events",
    external: { href: "/events", label: "View hire storefront" },
    items: [
      { href: "/events-ops", label: "Hire requests", icon: <I d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /> },
...
function ExternalNavLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="flex items-center gap-3 px-3 py-2 mt-0.5 rounded-lg text-sm font-medium text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-colors group"
    >
...
function NavSection({
  ...
  return (
    <div className="pt-4 first:pt-0">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400 hover:text-slate-600 transition-colors"
      >
  ...
  return (
    <nav className="flex-1 px-3 py-4 overflow-y-auto">
      <button
        ...
        className="w-full flex items-center gap-3 px-3 py-2.5 mb-3 rounded-lg text-sm font-medium text-slate-500 bg-slate-50 border border-slate-200 hover:border-slate-300 hover:bg-white hover:text-slate-700 transition-colors group"
      >
        ...
        <kbd className="hidden sm:inline text-[10px] font-semibold text-slate-400 bg-white border border-slate-200 rounded px-1.5 py-0.5">Ctrl K</kbd>
      </button>
    ...
    <div className="flex flex-col h-full bg-white border-r border-slate-200">
      ...
      <div className="px-5 py-4 flex items-center justify-between border-b border-slate-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-slate-900 font-semibold text-sm leading-tight">Dyzah</p>
      ...
      <div className="px-3 py-3 border-t border-slate-200">
        <div className="flex items-center gap-3 px-3 py-2 rounded-lg">
          <div className="w-8 h-8 bg-brand-50 border border-brand-100 rounded-full flex items-center justify-center shrink-0">
            <span className="text-brand-600 text-xs font-bold">
              {user?.name?.charAt(0)?.toUpperCase() ?? "A"}
            </span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-slate-800 text-sm font-medium leading-tight truncate">
              {user?.name ?? "Admin"}
            </p>
            <p className="text-slate-400 text-xs mt-0.5 truncate">{user?.role ?? "Administrator"}</p>
          </div>
          <button
            onClick={logout}
            title="Sign out"
...
export default function AppNav() {
  ...
  return (
    <>
      ...
      {/* Mobile top bar */}
      <div className="md:hidden sticky top-0 z-40 flex items-center justify-between px-4 py-3 bg-white border-b border-slate-200 shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-brand-500 rounded-xl flex items-center justify-center">
            <SpotlightIcon className="w-5 h-5 text-white" />
          </div>
          <span className="text-slate-900 font-semibold text-sm">Dyzah Admin</span>
        </div>
