    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-3">
          {items.map((item) => (
            <div key={item.id} className="flex gap-4 bg-white border border-slate-200 rounded-2xl p-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <div className="flex items-center border border-slate-300 rounded-lg overflow-hidden">
                    <button onClick={() => item.quantity > 1 && updateItem(item.id, item.quantity - 1)} className="px-2.5 py-1 text-slate-500 hover:bg-slate-100">−</button>
                    <span className="px-3 py-1 text-sm font-medium">{item.quantity}</span>
                    <button onClick={() => updateItem(item.id, item.quantity + 1)} disabled={!!item.variant && item.quantity >= item.variant.stock_qty} className="px-2.5 py-1 text-slate-500 hover:bg-slate-100 disabled:opacity-30">+</button>
                  </div>
                  <button onClick={() => removeItem(item.id)} className="text-xs text-slate-400 hover:text-red-500 font-medium">Remove</button>
                </div>
              </div>
              <div className="text-right shrink-0">
                <p className="font-bold text-slate-900">KSh {Number(item.line_total ?? 0).toLocaleString()}</p>
              </div>
            </div>
          ))}
        </div>
