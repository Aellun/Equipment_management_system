...
export default function StoreReviewsPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Store Reviews</h1>
      <p className="text-sm text-slate-500 mt-1">What our customers say about shopping with us.</p>

      {summary && summary.review_count > 0 && (
        <div className="flex items-center gap-3 mt-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5">
          <p className="text-4xl font-bold text-slate-900 dark:text-white">{summary.avg_rating?.toFixed(1)}</p>
          <div>
      ...
      <form onSubmit={submit} className="mt-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4">
        <p className="font-semibold text-slate-900 dark:text-white">Leave a review</p>
        <StarInput value={rating} onChange={setRating} />
        <div className="grid sm:grid-cols-2 gap-3">
          <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name *" className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm" />
          <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email (for Verified badge)" className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm" />
        </div>
        ...
        {reviews.length === 0 ? (
          ...
          <div key={r.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {r.verified_purchase && (
                <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded-full">
                  <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                  Verified Customer
                </span>
              )}
            </div>
            {r.body && <p className="text-sm text-slate-600 dark:text-slate-400 mt-1.5">{r.body}</p>}
