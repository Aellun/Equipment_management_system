    ...
    <header className="sticky top-0 z-40 bg-white shadow-sm">
      ...
      <div className="border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3 sm:gap-6">
          <Link href="/store" className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="leading-tight hidden sm:block">
              <p className="font-extrabold text-slate-900 text-lg tracking-tight">{storeName}</p>
          ...
          <form onSubmit={submitSearch} className="hidden md:flex flex-1 max-w-2xl mx-auto">
            <div className="flex w-full rounded-full border-2 border-orange-500 overflow-hidden bg-white focus-within:shadow-md transition-shadow">
              <select
                value={searchDept}
                onChange={(e) => setSearchDept(e.target.value)}
                className="hidden lg:block pl-4 pr-2 text-xs font-medium text-slate-500 bg-slate-50 border-r border-slate-200 focus:outline-none cursor-pointer"
              >
              ...
              <input
                ...
                className="flex-1 px-4 py-2.5 text-sm bg-transparent text-slate-900 focus:outline-none"
              />
          ...
          <div className="flex items-center gap-1 sm:gap-2 ml-auto md:ml-0">
            <Link href="/store/track" className="hidden sm:flex flex-col items-center px-3 py-1 text-slate-600 hover:text-orange-600 transition-colors">
            ...
            <Link href="/store/cart" className="relative flex flex-col items-center px-3 py-1 text-slate-600 hover:text-orange-600 transition-colors">
        ...
        <form onSubmit={submitSearch} className="md:hidden px-4 pb-3">
          <div className="flex w-full rounded-full border-2 border-orange-500 overflow-hidden bg-white">
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search products…"
              className="flex-1 px-4 py-2 text-sm bg-transparent text-slate-900 focus:outline-none"
            />
        ...
        <div className="max-w-7xl mx-auto px-4 h-10 flex items-center gap-1 text-sm">
          <div ref={catsRef} className="relative">
            <button
              ...
              className="flex items-center gap-2 px-3 h-10 font-semibold text-slate-800 hover:text-orange-600 transition-colors"
            >
            ...
            {catsOpen && (
              <div className="absolute left-0 top-full mt-px w-[560px] bg-white border border-slate-200 rounded-b-xl shadow-xl p-5 grid grid-cols-2 gap-6 animate-fadeIn">
                <div>
                  ...
                  <div className="space-y-0.5">
                    {departments.map((d) => (
                      <Link
                        ...
                        className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg text-slate-700 hover:bg-orange-50 hover:text-orange-600 transition-colors"
                      >
                  ...
                  <div className="grid grid-cols-1 gap-0.5 max-h-72 overflow-y-auto pr-1">
                    {categories.map((c) => (
                      <Link
                        ...
                        onClick={() => setCatsOpen(false)}
                        className="px-2 py-1.5 rounded-lg text-slate-600 hover:bg-orange-50 hover:text-orange-600 transition-colors text-[13px]"
                      >
                        {c.name}
                      </Link>
                    ))}
          ...
          <Link href="/store/browse" className="px-3 text-slate-600 hover:text-orange-600 font-medium transition-colors">All Products</Link>
          <Link href="/store/browse?sort=new" className="px-3 text-slate-600 hover:text-orange-600 font-medium transition-colors">New Arrivals</Link>
          <Link href="/store/browse?sort=rating" className="px-3 text-slate-600 hover:text-orange-600 font-medium transition-colors">Top Rated</Link>
          <Link href="/store/reviews" className="px-3 text-slate-600 hover:text-orange-600 font-medium transition-colors">Store Reviews</Link>
      ...
      <div className="md:hidden border-b border-slate-200 bg-white">
        <div className="flex gap-2 px-4 py-2 overflow-x-auto scrollbar-none">
          <Link href="/store/browse" className="shrink-0 px-3 py-1 rounded-full bg-orange-50 text-orange-700 text-xs font-semibold border border-orange-200">
            All
          </Link>
          {departments.map((d) => (
            <Link
              key={d.id}
              href={`/store/browse?dept=${d.slug}`}
              className="shrink-0 px-3 py-1 rounded-full bg-slate-50 text-slate-600 text-xs font-medium border border-slate-200"
            >
