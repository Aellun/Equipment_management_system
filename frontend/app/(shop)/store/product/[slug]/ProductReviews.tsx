...
export default function ProductReviews({ productId }: { productId: number }) {
  ...
  return (
    <div className="mt-12 border-t border-slate-200 dark:border-slate-800 pt-8">
      ...
      {showForm && (
        <form onSubmit={submit} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 mb-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Your rating</label>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email (for Verified Purchase badge)" className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm" />
          </div>
