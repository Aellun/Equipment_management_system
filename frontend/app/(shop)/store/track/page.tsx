...
function ReturnRequestBox({ orderNumber }: { orderNumber: string }) {
  ...
  if (done) {
    return (
      <div className="mt-6 border-t border-slate-100 pt-4 text-sm text-emerald-700">
    ...
    <div className="mt-6 border-t border-slate-100 pt-4">
      {!open ? (
        ...
        <form onSubmit={submit} className="space-y-3">
          <p className="text-sm font-semibold text-slate-700">Request a return — covered by our Genuine Guarantee</p>
          <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email used on the order *" className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 text-sm" />
          <select value={reason} onChange={(e) => setReason(e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 text-sm">
            {RETURN_REASONS.map((r) => <option key={r}>{r}</option>)}
          </select>
          <textarea rows={2} value={details} onChange={(e) => setDetails(e.target.value)} placeholder="Tell us more (optional)…" className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 text-sm resize-none" />
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <form onSubmit={(e) => { e.preventDefault(); lookup(code); }} className="flex gap-3 mt-5">
        <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="Enter order code" className="flex-1 bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-mono focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20" />
      ...
      {error && (
        <div className="mt-6 bg-red-50 text-red-700 rounded-xl px-4 py-3 text-sm">{error}</div>
      )}
      ...
      {order && (
        <div className="mt-8 bg-white border border-slate-200 rounded-2xl p-6">
          <div className="flex items-center justify-between flex-wrap gap-2 mb-6">
            ...
            <span className={`text-xs font-semibold uppercase px-3 py-1 rounded-full ${cancelled ? "bg-red-100 text-red-700" : "bg-orange-100 text-orange-700"}`}>{order.status}</span>
          </div>
              ...
              {STAGES.map((stage, i) => (
                <div key={stage} className="flex-1 flex items-center last:flex-none">
                  <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${i <= currentStage ? "bg-orange-600 text-white" : "bg-slate-200 text-slate-400"}`}>
                      {i < currentStage ? "✓" : i + 1}
                    </div>
                    <span className={`text-[10px] mt-1.5 ${i <= currentStage ? "text-slate-700 font-medium" : "text-slate-400"}`}>{stage}</span>
                  </div>
                  {i < STAGES.length - 1 && (
                    <div className={`flex-1 h-0.5 mx-1 -mt-4 ${i < currentStage ? "bg-orange-600" : "bg-slate-200"}`} />
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="mt-6 border-t border-slate-100 pt-4">
            ...
            {order.items.map((it) => (
              <div key={it.id} className="flex justify-between text-sm py-1">
                <span className="text-slate-600">{it.product_name} — {it.variant_name} ×{it.quantity}</span>
                <span className="text-slate-700">KSh {Number(it.line_total).toLocaleString()}</span>
              </div>
            ))}
          </div>
