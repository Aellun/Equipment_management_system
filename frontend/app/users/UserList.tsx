          ...
          <table className="w-full text-sm min-w-[480px]">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-800">
                ...
                <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider hidden sm:table-cell">Role</th>
                    ...
                    <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-4 py-3.5">
                        <div className="font-medium text-slate-900 dark:text-white flex items-center gap-2">
                          {u.name}
                          {isSelf && (
                            <span className="text-xs bg-indigo-100 text-indigo-600 dark:bg-indigo-900/40 dark:text-indigo-400 px-1.5 py-0.5 rounded-md font-medium">You</span>
                          )}
                        </div>
                        <div className="text-xs text-slate-400 dark:text-slate-500 mt-0.5 sm:hidden">{u.role}</div>
                      </td>
                      <td className="px-4 py-3.5 text-slate-600 dark:text-slate-300 text-sm truncate max-w-[180px]">{u.email}</td>
                      <td className="px-4 py-3.5 hidden sm:table-cell">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${u.role === "Administrator" ? "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-400" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300"}`}>
                          {u.role}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-right">
                        <button
                          ...
                          className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-slate-400 hover:text-red-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:text-slate-400 disabled:hover:bg-transparent"
                        >
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl w-full max-w-sm animate-fadeIn">
            <div className="p-6">
              ...
              <h3 className="text-base font-semibold text-slate-900 dark:text-white text-center">Delete user?</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 text-center mt-1.5">
                <span className="font-medium text-slate-700 dark:text-slate-200">{confirmName}</span> will be permanently removed and will no longer be able to log in.
              </p>
            </div>
