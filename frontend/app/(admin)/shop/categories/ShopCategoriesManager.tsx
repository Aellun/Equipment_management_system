    ...
    <div className="space-y-5">
      ...
      {categories.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Add one to organise your store products</p>
        </div>
      ) : (
          ...
          {categories.map((cat) => (
            ...
            >
              ...
              <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button onClick={() => setDeleteTarget(cat)} className="p-2 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 dark:hover:text-red-400 transition-colors" title="Delete">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl w-full max-w-sm animate-fadeIn">
            <div className="px-6 py-6 text-center">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Delete <strong className="text-slate-700 dark:text-slate-300">{deleteTarget.name}</strong>? Products keep existing but lose this category.
              </p>
            </div>
            <div className="flex gap-3 px-6 pb-6">
              ...
              <button onClick={() => setDeleteTarget(null)} className="flex-1 py-2.5 text-sm bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl transition-colors font-medium">
                Cancel
              </button>
            </div>
          </div>
