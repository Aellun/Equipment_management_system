    ...
    <div className="space-y-5">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
          <p className="font-semibold text-slate-700 dark:text-slate-300 text-sm">No products yet</p>
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Add your first kitchenware product to start selling</p>
        </div>
      ) : (
            ...
            return (
              <div key={p.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
                <div className="flex items-center gap-4 p-4">
                  <div className="w-14 h-14 rounded-xl bg-slate-100 dark:bg-slate-800 overflow-hidden shrink-0 flex items-center justify-center">
                    {p.images[0] ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {!p.is_active && <span className="text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-700 text-slate-500">Hidden</span>}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl w-full max-w-sm animate-fadeIn">
            <div className="px-6 py-6 text-center">
              <h3 className="font-semibold text-slate-900 dark:text-white mb-1">Delete Product</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Delete <strong className="text-slate-700 dark:text-slate-300">{deleteTarget.name}</strong> and all its variants? This cannot be undone.
              </p>
            </div>
...
function VariantEditor({ product, onRefresh }: { product: Product; onRefresh: () => void }) {
  ...
  return (
    <div className="border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30 px-4 py-3 space-y-2">
  ...
  return (
    <div className="grid grid-cols-12 gap-2 items-center text-sm">
      <div className="col-span-3 min-w-0">
        <p className="font-medium text-slate-800 dark:text-slate-200 truncate">{variant.variant_name}</p>
      ...
      <div className="col-span-2 flex items-center gap-1">
        ...
        <input type="number" step="0.01" value={price} onChange={(e) => setPrice(e.target.value)} className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-2 py-1 text-sm" />
      </div>
      <div className="col-span-2">
        <input type="number" value={stock} onChange={(e) => setStock(e.target.value)} className={`w-full border rounded-lg px-2 py-1 text-sm bg-white dark:bg-slate-800 ${Number(stock) === 0 ? "border-red-300 dark:border-red-800" : "border-slate-300 dark:border-slate-700"}`} />
      </div>
      <div className="col-span-3">
        <select
          ...
          className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-2 py-1 text-sm disabled:opacity-50"
