  ...
  if (returns.length === 0) {
    return (
      <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
        <p className="font-semibold text-slate-700 dark:text-slate-300 text-sm">No return requests</p>
      ...
      {returns.map((r) => (
        <div key={r.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              <p className="font-mono font-semibold text-slate-900 dark:text-white text-sm">{r.order_number ?? `Order #${r.order_id}`}</p>
          ...
          <p className="text-sm text-slate-700 dark:text-slate-300 mt-2"><strong>Reason:</strong> {r.reason}</p>
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            <select value={r.status} disabled={busy} onChange={(e) => patch(r, { status: e.target.value })} className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-1.5 text-sm">
