...
export default async function MaintenancePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Maintenance</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Open", value: open, sub: "awaiting action", tone: open > 0 ? "text-red-600 dark:text-red-400" : "" },
          ...
          <div key={c.label} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5">
            ...
            <p className={`text-3xl font-bold tracking-tight mt-2 ${c.tone || "text-slate-900 dark:text-white"}`}>{c.value}</p>
