  ...
  if (transactions.length === 0) {
    return (
      <div className="text-center py-14 bg-white border border-slate-200 rounded-2xl">
        ...
        <p className="text-sm text-slate-400 mt-1">Check out some equipment above to get started.</p>
      </div>
    );
  }
      ...
      <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div className="relative w-full sm:w-60">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all"
          />
        </div>
      </div>
        ...
        <table className="w-full text-sm min-w-[640px]">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50/70">
              ...
              <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider hidden md:table-cell">Out</th>
              <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Due</th>
                    ...
                    >
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2.5">
                          <svg className={`w-3.5 h-3.5 text-slate-300 transition-transform shrink-0 ${isExpanded ? "rotate-90" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                          <div className="min-w-0">
                            <p className="font-semibold text-slate-900 truncate max-w-[160px]">
                              {b.itemName}
                              <span className="ml-1.5 inline-flex items-center px-1.5 py-0.5 rounded-md bg-brand-100 text-brand-700 text-[11px] font-bold align-middle">
                                × {b.all.length}
                              </span>
                            </p>
                      ...
                      <td className="px-4 py-3.5">
                        <span className={`text-xs whitespace-nowrap ${overdue ? "text-red-600 font-semibold" : "text-slate-600"}`}>
                      ...
                      <td className="px-4 py-3.5 text-right whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                        {b.active.length > 0 ? (
                          <div className="inline-flex items-center gap-1.5">
                            ...
                            {overdue && b.clientEmail && (
                              <a
                                ...
                                className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-brand-100 text-brand-600 hover:bg-brand-200 transition-colors"
                              >
                                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          <span className="text-xs text-slate-400 font-medium">All returned</span>
                        )}
                      </td>
                    </tr>
                        ...
                        <td colSpan={6} className="px-4 py-3">
                          ...
                          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-1.5">
                            {b.all.map((t) => (
                              <div
                                key={t.id}
                                className="flex items-center justify-between gap-2 px-3 py-2 bg-white border border-slate-200 rounded-xl"
                              >
                                <div className="min-w-0">
...
function BatchCheckinModal({
  ...
  const conditionStyle: Record<ConditionOnReturn, string> = {
    ...
    "Needs Repair": "border-amber-500 bg-amber-50 text-amber-700",
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div className="bg-white border border-slate-200 rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-sm animate-slideInUp sm:animate-fadeIn max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 shrink-0">
          <div>
            <h3 className="font-semibold text-slate-900">Return Equipment</h3>
        ...
        <form onSubmit={submit} className="flex-1 overflow-y-auto">
          <div className="px-6 py-5 space-y-4">
            <div className="bg-slate-50 rounded-xl px-3.5 py-2.5 max-h-24 overflow-y-auto">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 resize-none transition-all"
              />
            </div>

            {condition !== "Good" && (
              <div className="flex items-start gap-2.5 text-xs bg-amber-50 border border-amber-200 rounded-xl px-3.5 py-3">
