...
const actionConfig: Record<string, { label: string; color: string }> = {
  ...
  bulk_delete: { label: "Bulk Delete", color: "bg-red-100 text-red-700" },
  bulk_update: { label: "Renamed", color: "bg-blue-100 text-blue-700" },
  checkout: { label: "Checked Out", color: "bg-amber-100 text-amber-700" },
  bulk_checkout: { label: "Bulk Checkout", color: "bg-amber-100 text-amber-700" },
  checkin: { label: "Returned", color: "bg-brand-100 text-brand-700" },
...
function ActionBadge({ action }: { action: string }) {
  const cfg = actionConfig[action] ?? { label: action, color: "bg-slate-100 text-slate-700" };
  ...
  if (logs.length === 0) {
    return (
      <div className="text-center py-16 bg-white border border-slate-200 rounded-2xl">
        ...
        <p className="font-semibold text-slate-700 text-sm">No activity recorded yet</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <select value={entityFilter} onChange={(e) => { setEntityFilter(e.target.value); setPage(1); }} className="bg-white border border-slate-300 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-brand-500 sm:w-40 transition-all">
          <option value="">All types</option>
          {uniqueEntities.map(e => <option key={e} value={e}>{entityConfig[e]?.label ?? e}</option>)}
        </select>
        <select value={actionFilter} onChange={(e) => { setActionFilter(e.target.value); setPage(1); }} className="bg-white border border-slate-300 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-brand-500 sm:w-40 transition-all">
          ...
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">When</th>
                <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Action</th>
                ...
                <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider hidden md:table-cell">Details</th>
                <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider hidden lg:table-cell">By</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {pageItems.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-slate-400">No log entries match your filters.</td>
                </tr>
              ) : pageItems.map((log) => (
                <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3 text-xs text-slate-500 whitespace-nowrap">
                    <div>{new Date(log.timestamp).toLocaleDateString()}</div>
                    <div className="text-slate-400">{new Date(log.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
                  </td>
                  <td className="px-4 py-3"><ActionBadge action={log.action} /></td>
                  <td className="px-4 py-3"><EntityBadge type={log.entity_type} /></td>
                  <td className="px-4 py-3">
                    <span className="font-medium text-slate-900 text-sm">{log.entity_name ?? `#${log.entity_id ?? "—"}`}</span>
                  </td>
