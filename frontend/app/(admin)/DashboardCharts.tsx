          ...
          {data.map((c) => {
            ...
            return (
              <div key={c.name}>
                ...
                <div className="flex h-2 rounded-full overflow-hidden bg-slate-100" style={{ width: `${(total / max) * 100}%`, minWidth: "15%" }}>
        ...
        <div className="space-y-3">
          {data.map((c, i) => (
            <div key={c.name} className="flex items-center gap-3">
              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${i === 0 ? "bg-brand-600 text-white" : "bg-slate-100 text-slate-500"}`}>
                {i + 1}
              </span>
              ...
              <span className="text-xs font-semibold text-slate-500 shrink-0">{c.count}</span>
            </div>
          ))}
        </div>
      )}
